...
def get_cosine_similarity(x, y):
    # x has shape (B, C*H*W), y has shape (N, C*H*W)
    # return x with shape (B, N)
...
class KnnFinder(nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, augment_labels=None):
        ...
        x = x.reshape(B, C*H*W)
        # compute score function
        distance_euc = - torch.cdist(x, y, p=2) ** 2 / 2 / sigma ** 2     # (B, N)
        descending_index_euc = torch.argsort(distance_euc, dim=-1, descending=True)
        distance_cos = get_cosine_similarity(x, y) #(B, N)
        descending_index_cos = torch.argsort(distance_cos, dim=-1, descending=True)
        #prob = torch.softmax(distance, dim=1)                         # softmax in the axis of N, (B, N)
        #denoised = prob @ y                                           # (B, C*H*W)
        #denoised = denoised.reshape(B, C, H, W)
        # score = (denoised - x) / sigma ** 2                                                    
        return descending_index_euc, descending_index_cos
